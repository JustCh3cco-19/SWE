...
#include <optimization/2025-09-08-ex4/simulation.hpp>
...
int main() {
  ...
  OptimizationResult result = OptimizeSuppliers(params, &rng);
  ...
  if (!mocc_utils::WriteResultsToFile("results.txt",
    ...
    output << "S " << result.best_suppliers << "\n";
    output << "C " << result.best_cost << "\n";
  }, &error)) {
    std::cerr << error << "\n";
    return 1;
  }
