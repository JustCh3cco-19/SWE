...
bool LoadParameters(const std::string& path, Parameters*out, std::string*error) {
  ...
  if (out->alpha == 0.0 && numeric_lines.size() >= 4 && numeric_lines[3].size() == 1) {
    out->alpha = numeric_lines[3][0];
  }
  if (p_line.empty() && numeric_lines.size() >= 5) {
    p_line = numeric_lines[4];
  }
  if (f_line.empty() && numeric_lines.size() >= 6) {
    f_line = numeric_lines[5];
  }
