#include <queues-and-services/2025-02-05-ex4/simulation.hpp>
...
double RunSimulation(const Parameters& params, int horizon, urng_t*rng) {
  ...
  for (int t = 0; t < horizon; ++t) {
    ...
    queue_sum += static_cast<double>(requests.size());
  ...
  return queue_sum / horizon;
}

}  // namespace

double EstimateAverageQueue(const Parameters& params, int simulations, urng_t*rng) {
  ...
  for (int i = 0; i < simulations; ++i) {
    total += RunSimulation(params, horizon, rng);
  }
